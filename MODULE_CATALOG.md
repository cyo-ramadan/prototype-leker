...
tabelnya sudah ada di `migrations/0039`, tinggal Langkah 4: pindahin isolasi
dari `store_id` ke `entity_id` di semua jalur baca/tulis). Program Ikan akan
diarahkan Bos Cyo langsung buat nyesuaiin ke mode itu dari sisi sesinya sendiri
— bukan dikerjakan dari sesi Leker ini. Saklar per-gerai yang lagi dibangun di
sini (`warehouse_enabled`, `edition`) tetap relevan buat arah itu: mekanismenya
sama persis dengan yang dibutuhkan buat saklar per-tenant nanti, cuma level
scope-nya beda.
