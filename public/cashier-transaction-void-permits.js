  ...
  async function openPermitList() {
    try {
      ...
      openDialog({
        ...
        body: rows.length ? `<div class="master-list">${rows.map((item, index) => `<article class="cashier-lock-note" style="margin-bottom:9px" data-void-row="${index}"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap"><div><b>${escapeHtml(labels[item.subjectType] || item.subjectType)} · ${escapeHtml(item.description)}</b><div class="muted">${escapeHtml(dt(item.createdAt))}${item.paymentMethod ? ` · ${escapeHtml(item.paymentMethod)}` : ''}</div></div><strong>${money(item.amount)}</strong></div><div class="muted" style="margin-top:7px">Permit: ${escapeHtml(permitLabel(item.permit))}${item.permit?.executionCode ? ` · ${escapeHtml(item.permit.executionCode)}` : ''}</div><button class="secondary-btn" type="button" data-request-void="${index}" style="margin-top:8px" ${canRequest(item) ? '' : 'disabled'}>🗑 Ajukan Hapus</button></article>`).join('')}</div>` : '<div class="empty">Belum ada Penjualan, Pembelian, atau Operasional pada laci aktif.</div>'
      });
