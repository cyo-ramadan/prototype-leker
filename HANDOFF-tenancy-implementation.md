...
nullable, backfill satu Entity per gerai di bawah `TEN-PROTOTYPE`). Langkah 4
("pindahkan isolasi dari `store_id` ke `entity_id`") **belum ada satu baris kode
aplikasi pun yang menyentuhnya** — `grep entity_id src/*.js` nol hasil, padahal
48 dari 63 file di `src/` pakai `store_id`. Ini bukan kerjaan satu task; kalau
dipaksa jadi satu task besar, hasilnya bakal seperti versi pertama
...
diklaim segera, boleh paralel satu sama lain. Fase 3 menyusul.
