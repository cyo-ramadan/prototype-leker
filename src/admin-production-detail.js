...
export async function handleAdminProductionDetailApi(request, env, pathname) {
  ...
  const run = await env.DB.prepare(`
    SELECT pr.id, pr.sale_id, pr.mode, pr.output_product_id, pr.output_product_name,
           ...
           pr.requested_sale_quantity, pr.hpp_total, pr.hpp_per_unit, pr.status,
  ...
  const transaction = {
    ...
    amount: run.hpp_total == null ? null : Number(run.hpp_total),
  ...
  return json({
    ...
    detail: {
      ...
      hppTotal: run.hpp_total == null ? null : Number(run.hpp_total),
      hppPerUnit: run.hpp_per_unit == null ? null : Number(run.hpp_per_unit),
      status: run.status,
