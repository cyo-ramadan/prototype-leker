(() => {
  ...
  function renderCostTypeOptions() {
  ...
  function renderCostTypeList() {
    el('costTypeList').innerHTML = snapshot.costTypes.length ? snapshot.costTypes.map(type => `<article class="master-row"><div class="master-main"><strong>${escapeHtml(type.name)}</strong><small>${type.isActive ? 'Aktif' : 'Nonaktif'}</small></div><button class="text-btn" type="button" data-edit-cost-type="${escapeHtml(type.id)}">Edit</button></article>`).join('') : '<div class="empty">Belum ada Jenis Biaya.</div>';
    document.querySelectorAll('[data-edit-cost-type]').forEach(button => button.onclick = () => editCostType(button.dataset.editCostType));
  }
  function editCostType(id) {
    const type = snapshot.costTypes.find(item => item.id === id);
    if (!type) return;
    el('costTypeId').value = type.id;
    el('costTypeName').value = type.name;
    el('costTypeActive').checked = type.isActive;
    el('costTypeTitle').textContent = 'Edit Jenis Biaya';
    el('costTypeCancel').classList.remove('hidden');
  }
  function resetCostType() {
    el('costTypeForm').reset();
    el('costTypeId').value = '';
    el('costTypeActive').checked = true;
    el('costTypeTitle').textContent = 'Tambah Jenis Biaya';
    el('costTypeCancel').classList.add('hidden');
  }
  async function saveCostType(event) {
    event.preventDefault();
    const id = el('costTypeId').value;
    const response = await api(id ? `/api/admin/master/cost-types/${encodeURIComponent(id)}` : '/api/admin/master/cost-types', {
      method: id ? 'PATCH' : 'POST',
      body: JSON.stringify({ name: el('costTypeName').value, isActive: el('costTypeActive').checked })
    });
    snapshot.costTypes = response.costTypes;
    toast('Jenis Biaya tersimpan');
    resetCostType();
    renderCostTypeOptions();
    renderCostTypeList();
  }
