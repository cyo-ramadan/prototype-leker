...
5. Make inventory purchases itemized by product, integer base-unit quantity, and line total.
...
8. Snapshot average cost into sale-item COGS and production component costs so later master changes cannot rewrite history.
9. Compute production HPP from component cost snapshots and fold produced output cost into the output product's moving average.
10. Keep Accounting journal generation outside Prototype Leker. Product Kind and cost snapshots are operational facts/reference data for future Accounting integration.

## Formula

For a purchased product with positive existing stock:

`new_average_cost = (old_stock × old_average_cost + purchase_line_total) / (old_stock + purchase_quantity)`

When existing stock is zero or less:

`new_average_cost = purchase_line_total / purchase_quantity`

Production output uses the same moving-average principle, replacing purchase line total with production run HPP and purchase quantity with produced quantity.

## Consequences

- Harga Beli Terakhir and Average Cost are read-only in Master Barang.
- Existing purchases must use item lines before they can affect inventory costing.
- Sale and production history carries immutable cost snapshots.
...
Migration `0019_product_costing_and_kinds.sql` is additive.
...
**REQUIRED** — contracts v2, migration 0019, Known Issues, regression tests, and this ADR are updated in the same change set.
