(() => {
  ...
  async function load() {
    ...
    el('costMasterType').innerHTML = snapshot.costTypes.filter(type => type.isActive).map(type => `<option value="${escapeHtml(type.id)}">${escapeHtml(type.name)}${type.accountingComponentLabel ? ` · ${escapeHtml(type.accountingComponentLabel)}` : ' · Accounting belum linked'}</option>`).join('');
