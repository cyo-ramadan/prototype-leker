...
function mapAttendance(row) {
  return { id: row.id, userId: row.user_id, storeId: row.store_id, type: row.attendance_type, photoType: row.photo_type, createdAt: row.created_at };
}

async function listAttendance(db, userId, limit = 60) {
  const rows = await db.prepare(`SELECT id, user_id, store_id, attendance_type, photo_type, created_at FROM staff_attendance WHERE user_id = ? ORDER BY created_at DESC LIMIT ?`).bind(userId, limit).all();
...
export async function handleStaffPortalApi(request, env, pathname) {
  ...
  if (request.method === 'POST' && pathname === '/api/staff/attendance') {
    ...
    await env.DB.prepare(`INSERT INTO staff_attendance (id, user_id, store_id, attendance_type, photo_blob, photo_type, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(id, auth.cashier.id, auth.cashier.store.id, attendanceType, photo.bytes, photo.type, now).run();
