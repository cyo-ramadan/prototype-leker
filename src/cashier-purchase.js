...
function normalizeItems(options, requested) {
  if (!Array.isArray(requested) || !requested.length || requested.length > 50) {
    return { ok: false, error: 'Pembelian wajib memiliki 1–50 baris barang.' };
  }
...
function purchaseItemStatements(db, {
  purchaseId, storeId, drawerId, cashierId, item, now
}) {
  ...
  return [
    ...
    db.prepare(`
      UPDATE products
      SET average_cost = (
            SELECT average_cost_after FROM purchase_items
    ...
    `).bind(
      itemId, purchaseId, storeId,
      itemId, purchaseId, storeId,
      itemId, purchaseId, storeId,
      now, now, item.productId, storeId
    ),
    db.prepare(`
      INSERT OR IGNORE INTO inventory_stock_balances (store_id, product_id, quantity, updated_at)
      VALUES (?, ?, 0, ?)
    `).bind(storeId, item.productId, now),
    db.prepare(`
      UPDATE inventory_stock_balances
      SET quantity = quantity + ?, updated_at = ?
      WHERE store_id = ? AND product_id = ?
    `).bind(item.quantity, now, storeId, item.productId),
    db.prepare(`
    ...
    `).bind(
      ...
      purchaseId,
      drawerId,
      `Pembelian · biaya baris ${item.lineTotal}`,
      cashierId,
      now
    )
  ];
...
export async function handleCashierPurchaseApi(request, env, pathname) {
  ...
  if (!paymentMethod) return json({ error: 'Cara bayar pembelian tidak valid.' }, 400);
  ...
  const description = text(body.value?.description, 220)
    || `Pembelian ${normalized.items.map(item => item.productName).slice(0, 4).join(', ')}`;
  ...
  for (const item of normalized.items) {
    statements.push(...purchaseItemStatements(env.DB, {
      purchaseId: id,
      storeId: cashier.store.id,
