...
function tableExists(name) {
  return Number(d1Json(`SELECT EXISTS(SELECT 1 FROM sqlite_schema WHERE type='table' AND name='${name}') AS ok;`)[0]?.ok || 0) === 1;
}

function columns(name) {
  return new Set(d1Json(`PRAGMA table_info(${name});`).map(row => String(row.name || '')));
}

function count(sql) {
  return Number(d1Json(sql)[0]?.row_count || 0);
}
...
const productRefs = columns('products').has('linked_recipe_id')
  ? count('SELECT COUNT(*) AS row_count FROM products WHERE linked_recipe_id IS NOT NULL;')
  : 0;
const saleRefs = columns('sale_items').has('recipe_id')
  ? count('SELECT COUNT(*) AS row_count FROM sale_items WHERE recipe_id IS NOT NULL;')
  : 0;
const productionRefs = tableExists('production_runs') && columns('production_runs').has('recipe_id')
  ? count('SELECT COUNT(*) AS row_count FROM production_runs WHERE recipe_id IS NOT NULL;')
  : 0;

console.log(JSON.stringify({
  recipeColumns: [...recipeCols],
  componentTableExists: componentExists,
...
if (canonicalRecipe && canonicalComponents) {
  ...
  if (!legacyRecipe) {
    throw new Error(`Unexpected manufacturing_recipes shape. Refusing recovery: ${JSON.stringify([...recipeCols])}`);
  }
  if (!legacyComponents && !canonicalComponents) {
    throw new Error(`Unexpected manufacturing_recipe_components shape. Refusing recovery: ${JSON.stringify([...componentCols])}`);
  }
...
CREATE UNIQUE INDEX idx_manufacturing_recipe_one_active_output
  ON manufacturing_recipes(store_id, output_product_id)
  WHERE status = 'ACTIVE';
CREATE INDEX idx_manufacturing_recipes_store_status_output
...
CREATE INDEX idx_manufacturing_components_recipe_order
  ON manufacturing_recipe_components(recipe_id, display_order, component_product_id);
CREATE INDEX idx_manufacturing_components_store_product
  ON manufacturing_recipe_components(store_id, component_product_id);

CREATE TRIGGER trg_manufacturing_recipe_scope
...
CREATE TRIGGER trg_manufacturing_component_scope
...
END;
`;
  console.log('Legacy Recipe tables are empty and unreferenced. Restoring exact canonical 0016 Recipe schema.');
  d1Exec(repairSql);
}
...
if (!verifiedRecipeCols.has('output_quantity') || verifiedRecipeCols.has('output_quantity_milli')) {
  throw new Error(`Recipe schema verification failed: ${JSON.stringify([...verifiedRecipeCols])}`);
}
if (!verifiedComponentCols.has('quantity') || verifiedComponentCols.has('quantity_milli')) {
  throw new Error(`Recipe component schema verification failed: ${JSON.stringify([...verifiedComponentCols])}`);
}
